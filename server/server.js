...
app.post("/pedidos", (req, res) => {
  ...
  (pedidoId, foto, nome, cor, tamanho, preco_total, quantidade, nome_cliente, telefone, CEP, bairro, rua_avenida, numero, complemento, forma_pagamento, status)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
      ...
      const values = [
        ...
        "pendente", // aqui o status padrão na inserção
      ];
...
// Atualizar status para "em entrega" (painel do admin)
app.put("/pedidos/:pedidoId/entregar", (req, res) => {
  const { pedidoId } = req.params;
  const sql = 'UPDATE pedidos SET status = "em entrega" WHERE pedidoId = ?';

  db.query(sql, [pedidoId], (err, result) => {
    if (err) {
      console.error('Erro ao atualizar status para "em entrega":', err);
      return res.status(500).send("Erro ao atualizar status");
    }

    res.status(200).json({ mensagem: 'Status atualizado para "em entrega".' });
  });
});

// Atualizar status para "entregue" (painel do entregador)
app.put("/pedidos/:pedidoId/entregue", (req, res) => {
  const { pedidoId } = req.params;
  const sql = 'UPDATE pedidos SET status = "entregue" WHERE pedidoId = ?';

  db.query(sql, [pedidoId], (err, result) => {
    if (err) {
      console.error('Erro ao atualizar status para "entregue":', err);
      return res.status(500).send("Erro ao atualizar status");
    }

    res.status(200).json({ mensagem: 'Status atualizado para "entregue".' });
  });
});
